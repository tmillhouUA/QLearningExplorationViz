...
let its = 0 //initializes counter to
let alpha = .1
let targetAlpha = .0001
let targetItsAlpha = 20000
let alphaUpdate = (targetAlpha/alpha)**(1/targetItsAlpha)

let epsilon = .5
let targetEpsilon = .001
let targetItsEpsilon = 20000
...
svg.append("text").text("Avoid The Sarlacc!").attr("dominant-baseline","middle").attr("text-anchor","middle").attr("font-weight","bold")
...
characters.append('image').attr('href', "../Qlearning_II/gamorrian.png")
                                    ...
                                    .attr("y",graphBorder+(graphEdge/maxY)+5)
...
function updateQValues(){
    ...
    alpha *= alphaUpdate
    if(its%50==0){console.log(`Iteration: ${its}`);console.log(`Alpha: ${alpha}`)}
...
function learn(){
    if(!learning){
        learnAnimation = setInterval(function (){updateFunction()},1)
